...
#configuration for retinex algorithm
...
def printResult(sortedBands, savedimg, resPos):
    x,y,w,h = resPos
    strVal = ""
    if (len(sortedBands) in [1,2,3,4,5]):
...
while True:
    ...
    if cv2.waitKey(1) & 0xFF == ord('q'):
        ...
        img_new = cv2.imread(path + '/saved_img.jpg')
        resClose = findResistors(img_new, rectCascade)

        for i in range(len(resClose)):
            sortedBands = findBands(resClose[i],DEBUG)
            cv2.imwrite(filename='detected_resistors.jpg', img=printResult(sortedBands, img_new, resClose[i][1]))
